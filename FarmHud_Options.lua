...
local function opt(info, value, ...)
	...
	elseif value ~= nil then
		if key == "MinimapIcon" then
			...
			if value then
				LibStub("LibDBIcon-1.0", true):Show(addon);
			else
				LibStub("LibDBIcon-1.0", true):Hide(addon);
			end
		else
			if (...) then
...
local function optTracking(info, value)
	local key = info[#info];
	if not key:find("^tracking^.+$") then return end
...
-- Forward declaration for refresh function (defined after options table)
local RefreshRangeCircleOptions

-- Build dynamic options for each range circle
local function BuildRangeCircleOptions()
	local args = {}
	if not FarmHudDB or not FarmHudDB.rangeCircles then return args end

	for i, circle in ipairs(FarmHudDB.rangeCircles) do
		local circleKey = "circle" .. i
		args[circleKey] = {
			type = "group",
			order = i,
			name = circle.name or ("Circle " .. i),
			inline = true,
			args = {
				show = {
					type = "toggle",
					order = 1,
					name = L["Show"] or SHOW,
					get = function() return FarmHudDB.rangeCircles[i].show end,
					set = function(_, value)
						FarmHudDB.rangeCircles[i].show = value
						if FarmHud.UpdateRangeCircles then FarmHud.UpdateRangeCircles() end
					end
				},
				name = {
					type = "input",
					order = 2,
					name = L["Name"] or NAME,
					get = function() return FarmHudDB.rangeCircles[i].name or "" end,
					set = function(_, value)
						FarmHudDB.rangeCircles[i].name = value
						LibStub("AceConfigRegistry-3.0"):NotifyChange("FarmHud")
					end
				},
				scale = {
					type = "range",
					...
					max = 1.5,
					step = 0.05,
					get = function() return FarmHudDB.rangeCircles[i].scale or 0.45 end,
					set = function(_, value)
						FarmHudDB.rangeCircles[i].scale = value
						if FarmHud.UpdateRangeCircles then FarmHud.UpdateRangeCircles() end
					end
				},
				color = {
					...
					hasAlpha = true,
					get = function()
						local c = FarmHudDB.rangeCircles[i]
						return c.r or 0, c.g or 1, c.b or 0, c.a or 0.5
					end,
					set = function(_, r, g, b, a)
						FarmHudDB.rangeCircles[i].r = r
						FarmHudDB.rangeCircles[i].g = g
						FarmHudDB.rangeCircles[i].b = b
						FarmHudDB.rangeCircles[i].a = a
						if FarmHud.UpdateRangeCircles then FarmHud.UpdateRangeCircles() end
					end
				},
				remove = {
					...
					confirm = true,
					confirmText = L["RemoveCircleConfirm"] or "Remove this circle?",
					func = function()
						table.remove(FarmHudDB.rangeCircles, i)
						if FarmHud.UpdateRangeCircles then FarmHud.UpdateRangeCircles() end
						RefreshRangeCircleOptions()
						LibStub("AceConfigRegistry-3.0"):NotifyChange("FarmHud")
					end
				}
			}
...
-- Reference to options table for dynamic updates
local options

-- Refresh the range circle options
RefreshRangeCircleOptions = function()
	if not options or not options.args or not options.args.rangeCircles then return end

	-- Remove old circle entries (keep desc and addCircle)
	for key in pairs(options.args.rangeCircles.args) do
		if key:match("^circle%d+$") then
			options.args.rangeCircles.args[key] = nil
		end
	end

	-- Add current circles
	local circleOptions = BuildRangeCircleOptions()
	for key, value in pairs(circleOptions) do
		options.args.rangeCircles.args[key] = value
	end
end
...
options = {
	...
	args = {
		...
		background = {
			...
			args = {
				...
				background_alpha = {
					type = "range",
					order = 1,
					name = L["BgTransparency1"],
					desc = L["BgTransparencyDesc1"],
					min = 0.0,
					max = 1,
					step = 0.1,
					isPercent = true,
				...
				background_alpha2 = {
					type = "range",
					order = 2,
					name = L["BgTransparency2"],
					desc = L["BgTransparencyDesc2"],
					min = 0.0,
					max = 1,
					step = 0.1,
					isPercent = true,
		...
		rangeCircles = {
			type = "group",
			order = 2,
			name = L["RangeCircles"] or "Range Circles",
			args = {
				desc = {
					type = "description",
					order = 0,
					name = L["RangeCirclesDesc"] or "Add and configure range circles on the HUD."
				},
				addCircle = {
					type = "execute",
					order = 1,
					name = L["AddCircle"] or "Add Circle",
					desc = L["AddCircleDesc"] or "Add a new range circle",
					func = function()
						if not FarmHudDB.rangeCircles then FarmHudDB.rangeCircles = {} end
						local num = #FarmHudDB.rangeCircles + 1
						table.insert(FarmHudDB.rangeCircles, {
							name = "Circle " .. num,
							show = true,
							scale = 0.45,
							r = 0,
		...
		tracking = {
			...
			args = {
				...
				townsfolk = {
					type = "group",
					order = 3,
					name = TOWNSFOLK_TRACKING_TEXT or "Townsfolk",
					args = {
						-- filled by function updateTrackingOptions
					},
					hidden = true,
		...
		debugging = {
			type = "group",
			order = 100,
			name = BINDING_HEADER_DEBUG or "Debugging",
			args = {
				info = {
					type = "description",
					order = 1,
...
local trackingPattern = "|T%s:26:26:0:0|t %s" -- Changed %d to %s for texture path support
...
function ns.RegisterOptions()
	...
	-- Ensure rangeCircles exists and has at least one circle
	if not FarmHudDB.rangeCircles or #FarmHudDB.rangeCircles == 0 then
	...
	-- Refresh range circle options now that FarmHudDB is initialized
	RefreshRangeCircleOptions()
...
-- AddonCompartmentFrame removed for 3.3.5a
