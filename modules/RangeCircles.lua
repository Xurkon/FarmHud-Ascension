...
local function UpdateCircle(parent,circleType,index,force)
	...
	ns.Mixin(circle, FarmHudCircleLineMixin)
    ...
    local visible = (cParent=="Minimap" and onMinimap) or (cParent=="FarmHud" and show)
	ns.SetShown(circle, visible);
	circle.dirty = nil
end
...
function FarmHudRangeCirclesMixin:Update(...)
	local hudShown = FarmHud:IsShown()
	self.currentParent = hudShown and "FarmHud" or "Minimap";
	
	-- Hide the entire frame if FarmHud is not shown (fixes circles showing without HUD)
	if not hudShown then
		self:Hide()
		return
	end
	
	-- Show the frame when FarmHud is shown
	self:Show()
    ...
    -- Fix for FarmHud parent being huge (7000+ px) causing off-screen circles
    if self.currentParent == "FarmHud" then
        local screenH = UIParent:GetHeight()
        if s > screenH then s = screenH end
    end
	self:SetSize(s,s);
...
function module.events.PLAYER_LOGIN()
	...
	if not FarmHudDB.defaultCircles[2] then
...
-- Polyfill CreateLine (Moved to Mixin scope)
function FarmHudCircleLineMixin:CreateLine()
    ...
    f.ag.rot = f.ag:CreateAnimation("Rotation")
    f.ag.rot:SetDuration(0)
    f.ag.rot:SetOrder(1)

    local wrapper = { frame = f, texture = f.Texture, ag = f.ag, rot = f.ag.rot }
    function wrapper:SetStartPoint(point, relativeTo, sx, sy)
        self.sx, self.sy = sx, sy
        self:Update()
    end
    function wrapper:SetEndPoint(point, relativeTo, ex, ey)
        self.ex, self.ey = ex, ey
        self:Update()
    end
    function wrapper:GetEndPoint()
        return "CENTER", nil, self.ex, self.ey
    end
    function wrapper:GetStartPoint()
        return "CENTER", nil, self.sx, self.sy
    end
    function wrapper:SetThickness(thickness)
        self.thickness = thickness
        self:Update()
    end
    function wrapper:SetColorTexture(r,g,b,a)
        self.texture:SetVertexColor(r,g,b,a)
    end
    function wrapper:Show() self.frame:Show() end
    function wrapper:Hide() self.frame:Hide() end
    function wrapper:IsShown() return self.frame:IsShown() end
    
    function wrapper:Update()
        if not (self.sx and self.sy and self.ex and self.ey) then return end
        local sx, sy, ex, ey = self.sx, self.sy, self.ex, self.ey
        ...
        self.frame:SetPoint("CENTER", relativeTo, "CENTER", cx, cy)
        self.frame:SetSize(length, self.thickness or 1)
        
        local angle = math.atan2(dy, dx)
        self.rot:SetRadians(angle)
        self.ag:Play()
        self.ag:Pause() -- Snap to end
    end
    return wrapper
end
...
-- Manually Mixin because XML loads before Lua in 3.3.5a
if FarmHudRangeCircles and FarmHudRangeCirclesMixin then
    ns.Mixin(FarmHudRangeCircles, FarmHudRangeCirclesMixin)
end
